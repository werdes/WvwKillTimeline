...
if ((isset($_SELECTION) && !isset($_ID)) || (isset($_SELECTION) && $_SELECTION == "single" && isset($_ID) && !empty($_ID))) {
    ...
    if (!$oConnection->connect_errno) {
        ...
        $cSqlStatement = "SELECT GROUP_CONCAT(match_slug.slug SEPARATOR '|') as slugs, world.arenanet_id as world_id, world.name as world_name, `match`.id as match_id, `match`.arenanet_id as match_arenanet_id, `match`.start_time as match_start, `match`.end_time as match_end, `match`.last_update as match_last_update,  match_worlds.color, match_worlds.host, sum(match_worlds.kills_sum) as kills, SUM(match_worlds.deaths_sum) as deaths from match_worlds inner join world on world.id = match_worlds.world_id inner join `match` on `match`.id = match_worlds.match_id inner join match_slug on match_slug.match_id = `match`.id  " . $cWhereClause . " GROUP BY match_worlds.id ORDER BY `match`.start_time desc, `match`.arenanet_id asc, match_worlds.color asc, match_worlds.host desc";
